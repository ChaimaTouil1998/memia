...
if (
    ...
    && !empty($_POST['adresse'])
    && !empty($_POST['date_expi'])
    && !empty($_POST['id'])

) {

    $sql = "UPDATE visiteurs SET nom=?, prenom=?, email=?, telephone=?, adresse=?, mp=? WHERE id=?";
    ...
    if ($req->rowCount() != 0) {
        $_SESSION['message']['text'] = "profil modifié avec succés";
        $_SESSION['message']['type'] = "success";
    } else {
        $_SESSION['message']['text'] = "Pas de modification ";
